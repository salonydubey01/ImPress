...
class GlobalImpalaToPresto(_GlobalTranslator):
    def __init__(self):
        self.from_language = "Impala"
    ...
    def _replace_double_quotes(self, query: str) -> str:
        """All double quotes in Impala are replaced by single quotes.
        ...
        Returns:
            str: Transformed SQL
        """
        return query.replace('"', "'")  # Double & single quotes are identical in Impala but different in Presto.

    def _replace_back_ticks(self, query: str) -> str:
        """Back ticks from Impala are replaced by double quotes in Presto. There are no back ticks in Presto.
    ...
    def _increment_array_indexes(self, query: str) -> str:
        """Arrays indexing is 1 based on Presto while it's 0 based in Impala.
    ...
    def _fix_lateral_view_explode_calls(self, query: str) -> str:
        """Lateral view explode in Impala is translated by CROSS JOIN UNNEST in Presto. 
    ...
    def _fix_double_equals(self, query: str) -> str:
        """Impala tolerates single or double equales in comparison but Presto is single equal only.
    ...
    def move_insert_statement(self, sql: str) -> str:
        """Move the Impala insert statement (which is before the final select) all the way to the top of the file.
        ...
        pattern = r"(?P<insert_statement>{base_pattern})".format(base_pattern=r"\s+".join(utils.regex_impala_insert))
        try:
            ...
            pattern = r"(?P<insert_statement>{base_pattern})".format(base_pattern=utils.regex_impala_insert[0])
            result = Regex.search(pattern, sql)
