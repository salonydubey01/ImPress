...
Regex = regex.Regex()
SpecialFunctionHandlerImpalaToPresto = special_functions_handling.SpecialFunctionHandlerImpalaToPresto()
...
class RecursiveImpalaToPresto(_RecursiveTranslator):
    def __init__(self):
        ...
        self.functions = {}
        path_files = Path(os.path.join(os.path.dirname(__file__), "..", "function_dictionaries", "impala_to_presto")).rglob("*.json")
    ...
    def translate_query(self, query: str, has_insert_statement: bool = True) -> str:
        """Translate an entire query from Impala to Presto
        ...
        if has_insert_statement:
            _, _, _, self.partition_info = utils.parse_impala_insertion(query)
        else:
            self.partition_info = {}
        # Breakdown the query
    ...
    def _translate_function(self, tokens: List, verbose=False) -> Tuple[str, str]:  # A function call has been identified. Could be nested
        """Translates a Impala function to its Presto equivalent.
        ...
        elif SpecialFunctionHandlerImpalaToPresto.special_functions.get(function_name):  # Special function management
            translated_function_call = SpecialFunctionHandlerImpalaToPresto.special_functions[function_name](
