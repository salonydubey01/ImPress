...
def get_path_active_impala_files(paths_job_folders: List[str], path_default_udf: str = "") -> List[Dict]:
    """Get the list of active Impala files to be translated.

    Args:
        paths_job_folders (List[str]): List of folder to explore to retrieve impala SQL files
    ...
    Returns:
        ...
        {
            "impala": absolute path to the Impala file,
    ...
    for path_folder in tq(paths_job_folders):
        ...
        # Collect all impala files & generate their presto path
        file_names_to_consider = [
            file_name
            for file_name in config["run_order"]
            if file_name.endswith(".impala") and not "ddl" in file_name
        ]
        for file_name in file_names_to_consider:
            ...
            for query in sqlparse.parse(sql):  # Extract all queries from statement
                ...
                path_info.append({
                    "impala": absolute_path,
                    "presto": Regex.sub(r".impala$", ".presto", absolute_path),
                    "config": path_config,
                    "udf": path_udf
                })
...
def parse_describe_formatted(query_output: List[Tuple[str]]) -> Dict:
    """Converts to json like format the output of Impala's DESCRIBE FORMATTED
...
def parse_impala_insertion(sql: str) -> Tuple[str, str, str, Dict]:
    """Parse the insert statement in the Impala SQL
    ...
    # II. Regex searches
    pattern = r"\s+".join(regex_impala_insert)
    ...
    try:
        ...
        result = Regex.search(regex_impala_insert[0], sql)
...
def format_column_name_impala(column_name: str) -> str:
    """Surround column name with back ticks if needed to make it Impala compatible.
