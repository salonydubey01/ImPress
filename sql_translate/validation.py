...
class ImpalaToPresto(_Validator):
    def __init__(
        self,
        ...
        valid_impala_table_names = r"[\w\d\{\}]+|\`.+\`"
        self.regex_impala_insert = utils.regex_impala_insert
        ...
        self.ImpalaTableExplorer = utils.ImpalaTableExplorer(hconn)
        self.ErrorHandlerImpalaToPresto = error_handling.ErrorHandlerImpalaToPresto()

    def set_paths(self, path_src_sql: str, path_target_sql: str) -> None:
        """Save the paths to the source (Impala) & target (Presto) SQL
    ...
    def get_and_create_table_properties(self, database: str):
        ...
        _, src_table_db, src_table_name, _ = utils.parse_impala_insertion(self.src_sql.replace("${", "{"))
        ...
        self.src_table_properties = self.ImpalaTableExplorer.get_table_properties(f"{src_table_db}.{src_table_name}")

        # II. Create (nearly identical) table properties for temp src & temp tgt tables
        self.temp_src_table_properties = {
            "name": f"validation_impala_{self.src_table_properties['name']}",  # Does NOT have the database name
    ...
    def create_sandbox_tables(self) -> None:
        """Wrapper around _create_sandbox_table to create both Impala & Presto sandbox tables.
        - the Impala table is merely created executing the source Impala SQL
        - the Presto table is created by executing the translation
        """
        # I. Create the temporary Impala table
    ...
    def _create_sandbox_table(self, table_name: str, column_info: Dict[str, str], partition_info: Dict[str, str], engine: str) -> None:
        ...
        Args:
            ...
            engine (str): Either Impala or Presto
        """
        ...
        if engine.lower() == "impala":
            column_info = {utils.format_column_name_impala(k): self.upscale_integers(v) for k, v in column_info.items()}
            partition_info = {utils.format_column_name_impala(k): self.upscale_integers(v) for k, v in partition_info.items()}
    ...
    @validation_runner
    def insert_into_impala_table(self) -> float:
        """Insert into the Impala validation table in sandbox
        ...
        if self.temp_src_table_properties["latest_partitions"]:
            ...
            dynamic_partitioning = False if Regex.search(r"\s+".join(self.regex_impala_insert), sql).groupdict().get("partition_value") else True
            ...
            sql = Regex.sub(
                r"\s+".join(self.regex_impala_insert),
            ...
            sql = Regex.sub(
                self.regex_impala_insert[0],
                f"INSERT OVERWRITE TABLE {self.test_database}.{self.temp_src_table_properties['name']}",
                sql
            )

        # print(f"Insert into Impala table with:\n{sql}")

        # III. Execute query
        if dynamic_partitioning:
            run_query("SET impala.exec.dynamic.partition.mode=nonstrict", self.hconn)
        else:  # Should be the default
            run_query("SET impala.exec.dynamic.partition.mode=strict", self.hconn)
        start = time.perf_counter()
        run_query(sql, self.hconn)
        duration = time.perf_counter() - start
        print(f"Data was inserted in temp table in {duration:.3f} s with Impala")
        return duration
        ...
        while True:
            ...
            try:
                ...
                if error_message == "Error with HTTP request, response code: 500":
                    ...
                    sql, original_sql = self.ErrorHandlerImpalaToPresto.handle_errors(sql, original_sql, error_message, temp_tgt_table_properties=self.temp_tgt_table_properties)
    ...
    def validate_dml(self, path_original: str, path_translation: str, path_udf: str, path_config: str, database: str) -> Tuple[float, float, float]:
        ...
        Args:
            path_original (str): path of the source (Impala) SQL
        ...
        Returns:
            ...
            - Time it took to run the Impala query
        ...
        impala_run_time = self.insert_into_impala_table()
        print(f"Successfully inserted into {self.temp_src_table_properties['name']} with Impala")

        # IV. Compare Presto to Impala table based on metrics
        return self.compare_tables(), impala_run_time, presto_run_time


class TableComparator():  # Should be mostly Impala/Presto agnostic
