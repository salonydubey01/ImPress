...
def test_get_path_active_impala_files() -> None:
    paths_job_folders = [
        os.path.join(os.path.dirname(__file__), "samples", "example_folder")
    ]
    assert utils.get_path_active_impala_files(paths_job_folders) == [
        {
            "impala": os.path.join(paths_job_folders[0], "test.impala"),
...
def test_format_column_name_impala(column: str, expected: float) -> None:
    assert utils.format_column_name_impala(column) == expected
...
def test_parse_impala_insertion(path_file: str, expected: Tuple[str, Dict]) -> None:
    with open(os.path.join(PATH_SAMPLES, "parse_impala_insertion", path_file)) as f:
        sql = f.read()
    assert utils.parse_impala_insertion(sql) == expected
...
def test_get_latest_partitions(mock_fetch: MagicMock, partitions: List[Tuple[str]], expected: str) -> None:
    ImpalaTableExplorer = utils.ImpalaTableExplorer("")
    ImpalaTableExplorer.decode_utf8 = MagicMock(side_effect=lambda x: x)  # Return input without change
    mock_fetch.return_value = partitions
    ImpalaTableExplorer._get_latest_partitions("")
...
def test_describe_formatted(mock_fetch: MagicMock, mock_parse_describe_formatted: MagicMock) -> None:
    ...
    ImpalaTableExplorer._describe_formatted("a") == {"a", "b"}
...
def test_get_table_properties(path_describe_formatted_output: Dict, expected: Dict) -> None:
    ...
    ImpalaTableExplorer._describe_formatted = MagicMock(return_value=describe_formatted_output)
    ImpalaTableExplorer._get_latest_partitions = MagicMock(return_value={"date_timestamp": "1"})
    ImpalaTableExplorer.get_table_properties("db.my_table") == expected
