...
Translation = recursive_translation.RecursiveImpalaToPresto()
...
@pytest.mark.parametrize(['statement', 'translation', 'output_type'], [
    ...
    ("zzz_test_no_args_impala()", "zzz_test_no_args_presto()", "any"),
...
def test_breakdown_query(statement: str, expected: str) -> None:
    Translation = recursive_translation.RecursiveImpalaToPresto()
    query = sqlparse.parse(statement)[0]  # Single query in this statement
    _, _, _, Translation.partition_info = utils.parse_impala_insertion(query.value)  # Set the partition information
