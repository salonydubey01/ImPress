...
def test_set_paths() -> None:
    Validator = validation.ImpalaToPresto("", "", "", "")
    path_src = os.path.join(PATH_SAMPLES, "translation", "complex_statement.impala")
...
def test_evaluate_udfs() -> None:
    Validator = validation.ImpalaToPresto("", "", "", "")
    Validator.path_src_sql = os.path.join(PATH_SAMPLES, "translation", "complex_statement.impala")
...
@patch('sql_translate.validation.utils.parse_impala_insertion', return_value=("", "", "temp_table", ""))
def test_get_and_create_table_properties(mock_parse_impala_insertion: MagicMock) -> None:
    Validator = validation.ImpalaToPresto("", "", "", "")
    Validator.src_sql = ""
    Validator.ImpalaTableExplorer.get_table_properties = MagicMock(return_value=TABLE_PROPERTIES_PARTITIONED)
...
def test_create_sandbox_table(mock_run_query: MagicMock) -> None:
    Validator = validation.ImpalaToPresto("", "", "test_db", "")
    table_name = "my_table"
...
@patch('sql_translate.validation.run_query', return_value=None)
def test_insert_into_impala_table(mock_run_query: MagicMock) -> None:
    # Set up
    Validator = validation.ImpalaToPresto("", "", "test_db", "")
    ...
    Validator.insert_into_impala_table()
    ...
    # Check
    expected_calls = [
        call("SET impala.exec.dynamic.partition.mode=strict", ""),
        ...
        call("SET impala.exec.dynamic.partition.mode=nonstrict", ""),
...
def test_validate_dml() -> None:
    ...
    Validator.insert_into_impala_table = MagicMock(return_value=("", 1.1))
